...
BASE_COLOR = (255, 140, 84)
NOVEL_COLOR = (255, 255, 0)
...
class FewShotNWPUV2Dataset(BaseFewShotDataset):
    ...
    def __init__(self,
        ...
        self.PALETTE = self._build_palette(self.CLASSES)
    ...
    def _build_palette(self, classes: Sequence[str]) -> List[tuple]:
        """Build class-wise palette with one color for base classes and one
        color for novel classes.

        Args:
            classes (Sequence[str]): Current dataset classes.

        Returns:
            List[tuple]: Palette aligned with `classes`.
        """
        if self.split_id is None:
            # if split_id is unavailable, fall back to one uniform color
            return [BASE_COLOR for _ in classes]

        novel_classes = set(self.SPLIT[f'NOVEL_CLASSES_SPLIT{self.split_id}'])
        palette = []
        for cls_name in classes:
            if cls_name in novel_classes:
                palette.append(NOVEL_COLOR)
            else:
