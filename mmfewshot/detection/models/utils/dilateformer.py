...
class DilateAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        # B, C//3, H, W
        B, d, H, W = q.shape
        q = q.reshape([B, d // self.head_dim, self.head_dim, 1, H * W]).permute(0, 1, 4, 3, 2).contiguous()  # B,h,N,1,d
        k = self.unfold(k).reshape(
            [B, d // self.head_dim, self.head_dim, self.kernel_size * self.kernel_size, H * W]).permute(0, 1, 4, 2, 3).contiguous()  # B,h,N,d,k*k
        attn = (q @ k) * self.scale  # B,h,N,1,k*k
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        v = self.unfold(v).reshape(
            [B, d // self.head_dim, self.head_dim, self.kernel_size * self.kernel_size, H * W]).permute(0, 1, 4, 3, 2).contiguous()  # B,h,N,k*k,d
        x = (attn @ v).transpose(1, 2).reshape(B, H, W, d).contiguous()
        return x
...
class MultiDilatelocalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x.clone()).reshape(B, 3, self.num_dilation, C // self.num_dilation, H, W).permute(2, 1, 0, 3, 4, 5).contiguous()
        ...
        x = x.permute(1, 2, 3, 0, 4).contiguous().reshape(B, H, W, C)
        # 每个分支单独算 qkv，用 unfold 做局部邻域采样
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class GlobalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, H, W, C).contiguous()
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
